...
const Inbox = () => {
    ...
    const fetchEmployees = async () => {
      try {
        ...
        const employeesData = employeesSnapshot.docs.map((doc) => ({
          id: doc.id,
          displayName: doc.data().displayName || "Desconocido",
          role: doc.data().role || "Operador", // Por defecto operador si no se define el rol
          ...doc.data(),
        }));
  ...
  const saveAlert = async (alertData) => {
    ...
    await addDoc(alertRef, alertData);
  };
  ...
  const handleUpdateResponse = async () => {
    ...
    try {
      ...
      setTickets((prevTickets) =>
        prevTickets.map((ticket) =>
          ticket.id === selectedTicket.id
            ? { ...ticket, respuesta: response, updatedAt: new Date() }
            : ticket
        )
      );
      handleClose();
      ...
      toast.error(
        "Error al actualizar la respuesta. Por favor, intenta de nuevo."
      );
    }
  };
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setTickets((prevTickets) =>
        prevTickets.map((ticket) =>
          ticket.id === selectedTicket.id
            ? { ...ticket, assignedTo: assignTo, updatedAt: new Date() }
            : ticket
        )
      );
      handleClose();
  ...
  const columns = [
    ...
    // Línea modificada dentro de `columns` para la columna "Asignado a"
    {
      field: "assignedTo",
      ...
      valueGetter: (params) => {
        ...
        const assignedEmployee = employees.find(
          (employee) => employee.id === params.row.assignedTo
        );

        if (!assignedEmployee) return "Sin asignar";

        return `${assignedEmployee.displayName || "Desconocido"} - ${
          assignedEmployee.role === "Administrador"
            ? "Administrador"
            : "Operador"
        }`;
      },
    ...
    <div className="inbox-container">
      ...
      <Box sx={{ mb: 2, display: "flex", justifyContent: "flex-end" }}>
        <FormControl variant="outlined" size="small" sx={{ minWidth: 200 }}>
          <InputLabel id="filter-assign-label">
            ...
            <>
              ...
              <Typography sx={{ mt: 1 }}>
                ...
                {selectedTicket.assignedTo
                  ? `${
                      employees.find(
                        (employee) => employee.id === selectedTicket.assignedTo
              ...
              <Typography sx={{ mt: 1 }}>
                <strong>Estado de Respuesta:</strong>{" "}
                {selectedTicket.estado || "En espera"}
              </Typography>
